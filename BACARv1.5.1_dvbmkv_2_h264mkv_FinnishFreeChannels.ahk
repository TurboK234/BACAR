...
allowmultiplestreamhits := "yes"										; Optional: Using "yes" you can skip the precaution of checking that the rule only finds one matching stream. Use only if the files are known to have indistinguishable streams (and both are can be accepted).
