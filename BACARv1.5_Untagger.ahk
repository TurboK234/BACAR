...
; This script is used to remove "file already converted" tags that BACAR script has created for the source files.
; The tags are very useful for automated setups, especially when the user does not want to delete the converted files
; immediately after the conversion. The tags prevent the file to be converted over and over, but there are times when
; the user wants to do this, especially when setting up the system, when (for example) the parameters for conversion
; change and the conversion is repeated. Tags can also be removed manually when there are only a few files,
; but "attribute" tag is rather difficult to remove with normal Windows tools. That method was the original reason
; for creating this automated script. You can remove all tags with conversiontag_method := "all" easily, and it is
; the default value.
...
conversiontag_method := "all"											; This needs to be one of the supported methods (either "rename", "attribute" or "logfolder") or "all". This defines which files get "untagged". Use the same method as with original tagging. "all" removes all tags.
...
; MESSAGE BOX ABOUT FOUND FILES AND QUERY TO CONTINUE WITH UNTAGGING
GoSub, query_count_continue

; THE ACTUAL UNTAGGING OF THE FILES
GoSub, untag_conversion_status
...
query_runscript:
{
	MsgBox, 1, CHECKING,
	(LTrim
		The conversion status of the selected files
		is about to be reset. The process will start
		in 10 seconds.
...
init_global:
{
	...
	If (conversiontag_method = "rename" or conversiontag_method = "attribute" or conversiontag_method = "logfolder" or conversiontag_method = "all")
	{
...
{
	MsgBox, 1, COUNT BEFORE UNTAGGING,
	(LTrim
		Before untagging, there are total of %count_total_files% files
		...
		You are about to untag with method "%conversiontag_method%" .
		You can start untagging now by selecting "OK".
...
untag_conversion_status:
{
	Loop, %dir_rec%\*.%extension_rec%
	{
		If (conversiontag_method = "attribute" or conversiontag_method = "all")
		{
			FileSetAttrib, -T, %A_LoopFileName%
		}
		If (conversiontag_method = "logfolder" or conversiontag_method = "all")
		{
			IfExist, %dir_rec%\bacar_log\log_data\%A_LoopFileName%.txt
			{
		...
		If (conversiontag_method = "rename" or conversiontag_method = "all")
		{
			IfInString, A_LoopFileName, _CONVERTED
			{
				StringLen, length_extension_rec, extension_rec
				length_extensionplusperiod_rec := (length_extension_rec + 1)
				StringTrimRight, filenamebody_converted, A_LoopFileName, %length_extensionplusperiod_rec%
				StringGetPos, location_converted, filenamebody_converted, _CONVERTED
				StringLeft, filenamebody_untagged, filenamebody_converted, location_converted
				RunWait, %comspec% /c rename "%dir_rec%\%A_LoopFileName%" "%filenamebody_untagged%.%extension_rec%"
				Sleep 200
			}
		}
	}
...
info_result:
{
	...
	(LTrim
		After untagging, there are total of %count_total_files%
		with the extension .%extension_rec% in
		the directory %dir_rec% .
