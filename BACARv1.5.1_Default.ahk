...
conversiontag_method := ""											; This needs to be one of the supported methods (either "rename", "attribute" or "logfolder"). This defines how the script recognizes (and tags) already converted source files. Avoid switching!
...
show_conversion_confirmation = yes									; Show a OK/Cancel dialogue for each file before each file's encoding process, shows the ffmpeg command to be executed. Not suitable for automated work flow, good for setting the system up.
...
avoid_repetitive_log_events = no										; Selecting yes here avoids logging expected events that might be interesting when you want to know how/why the script does certain things (like "skipped the file cause it was already converted"), but after setting up these events only cause repetitive logging.
...
; Extraction rule title: *** Placeholder Rule To Be Copied and Filled ***	; non-formal title for user reminder
...
allowmultiplestreamhits := ""										; Optional: Using "yes" you can skip the precaution of checking that the rule only finds one matching stream. Use only if the files are known to have indistinguishable streams (and both are can be accepted).
	...
	While, (A_Index <= streamextr_rulecount)
	{
		...
		Loop, Read, %dir_temp%\ffprobereport.log
		{
			...
			IfInString, A_LoopReadLine, % extraction_line_rule_1_streamextr_%stream_current_index%
			{
				...
				{
					If (current_stream_found < 1 or allowmultiplestreamhits_streamextr_%stream_current_index% = "yes")
					{
			...
			If (current_stream <> "")			; if the stream was found, i.e. it was found on ONE line (unless allowmultiplestreamhits = "yes" for the rule) that meets the rules (line_rule_1 and line_rule_2), then create a mapping argument for FFmpeg.
			{
		...
		If (stream_mapped < 1)
		{
			FileAppend, % A_DD "/" A_MM "/" A_YYYY " " A_Hour ":" A_Min ":" A_Sec " : Stream(s) with search index " stream_current_index " was not found. `n", %dir_rec%\conversionlog-%current_year%.txt
			stream_mapped = 
			current_stream =
			continue
		}
...
init_streamindex:
{
	...
	allowmultiplestreamhits_%streamextr_tag% = %allowmultiplestreamhits%
