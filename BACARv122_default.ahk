...
Loop, %dir_rec%\*.%extension_rec%
{
	...
	{
		IfInString, ffprobereport, %global_ffprobereport_iff%
		{
	...
	{
		IfInString, ffprobereport, %global_ffprobereport_ifnot%
		{
	...
	While, (A_Index <= metadata_rulecount)
	{
		...
		{
			IfInString, ffprobereport, % extractionrule_ffprobereport_iff_metadata_%metadata_current_index%
			{
		...
		{
			IfInString, ffprobereport, % extractionrule_ffprobereport_ifnot_metadata_%metadata_current_index%
			{
			...
			{
				FileAppend, % A_DD "/" A_MM "/" A_YYYY " " A_Hour ":" A_Min ":" A_Sec " : Metadata (index " metadata_current_index ") specific rule (based on FFprobe's report) for rule exclusion was not met, skipping file " A_LoopFileName ". `n", %dir_rec%\conversionlog-%current_year%.txt
			}
		}
	...
	While, (A_Index <= streamextr_rulecount)
	{
		...
		{
			IfInString, ffprobereport, % extractionrule_ffprobereport_iff_streamextr_%stream_current_index%
			{
		...
		{
			IfInString, ffprobereport, % extractionrule_ffprobereport_ifnot_streamextr_%stream_current_index%
			{
			...
			{
				FileAppend, % A_DD "/" A_MM "/" A_YYYY " " A_Hour ":" A_Min ":" A_Sec " : Stream search (index " stream_current_index ") specific rule (based on FFprobe's report) for rule exclusion was not met, skipping file " A_LoopFileName ". `n", %dir_rec%\conversionlog-%current_year%.txt
			}
		}
