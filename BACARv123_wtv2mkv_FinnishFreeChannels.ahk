...
days_keepold = 30													; # of days to keep original (succesfully converted) recordings, older files are deleted. Use -1 to disable deleting.
...
show_conversion_confirmation = no									; Show a OK/Cancel dialogue for each file before each file's encoding process, shows the ffmpeg command to be executed. Not suitable for automated workflow, good for setting up.
...
global_codec_encoder_audio := "libmp3lame"									; Parameter that is passed to ffmpeg after option -codec:a
...
extractionrule_ffprobereport_iff := "service_provider: YLE"								; Sets a single string to look for in ffprobe's report for each processed file, and apply this rule in only those files. (Logical iff)
...
extraction_type = streamindex													; Type of data to extract (streamindex/metadata_nfo supported)
extraction_line_rule_1 := "impaired"										; The "fingerprint" data in the ffprobe's report that distinguishes the selected line from every other line. Try to find a unique string.
...
; <----------------------------------------------------------------------
; Copy each section starting from the line above
; Extraction rule title: *** Exclude "mjpeg" video stream ("thumbnail") ***		; non-formal title for user reminder
extraction_type = streamindex													; Type of data to extract (streamindex/metadata_nfo supported)
extraction_line_rule_1 := "Video: mjpeg"										; The "fingerprint" data in the ffprobe's report that distinguishes the selected line from every other line. Try to find a unique string.
...
extraction_type = metadata_nfo												; Type of data to extract (streamindex/metadata_nfo supported)
extraction_line_rule_1 := "genre           :"										; The "fingerprint" data in the ffprobe's report that distinguishes the selected line from every other line. Try to find a unique string.
extraction_line_rule_2 := ""										; Optional: User can fill here another rule that is REQUIRED TO COEXIST WITH rule_1 on the SAME LINE for the same extraction process (logical "AND").
line_exclusion_rule_1 := ""											; Optional: User can fill here one string that causes the line to be skipped if found. Handy, if line_rule_1 and line_rule_2 are inadequate.
stream_include_or_exclude := ""										; Used by ffmpeg to set a "negative mapping" (exclusion) if "exclude"
...
; <----------------------------------------------------------------------
; Copy each section starting from the line above
; Extraction rule title: *** Extract show original year/date from metadata ***		; non-formal title for user reminder
extraction_type = metadata_nfo												; Type of data to extract (streamindex/metadata_nfo supported)
extraction_line_rule_1 := "WM/OriginalReleaseTime:"										; The "fingerprint" data in the ffprobe's report that distinguishes the selected line from every other line. Try to find a unique string.
extraction_line_rule_2 := ""										; Optional: User can fill here another rule that is REQUIRED TO COEXIST WITH rule_1 on the SAME LINE for the same extraction process (logical "AND").
line_exclusion_rule_1 := ""											; Optional: User can fill here one string that causes the line to be skipped if found. Handy, if line_rule_1 and line_rule_2 are inadequate.
stream_include_or_exclude := ""										; Used by ffmpeg to set a "negative mapping" (exclusion) if "exclude"
metadata_xml_header := "year"											; Required only for .nfo metadata extraction, gives the field an appropriate header in the target .nfo -file. Don't fill <> here, only the text.
extractionrule_filename_iff := ""									; Sets a single string to look for in each processed filename, and apply this rule in only those files. (Logical iff)
extractionrule_filename_ifnot := ""									; Sets a single string to look for in each processed filename, and don't apply this rule with those files. (Logical if not)
extractionrule_ffprobereport_iff := ""								; Sets a single string to look for in ffprobe's report for each processed file, and apply this rule in only those files. (Logical iff)
extractionrule_ffprobereport_ifnot := ""							; Sets a single string to look for in ffprobe's report for each processed file, and don't apply this rule with those files. (Logical if not)
...
; <----------------------------------------------------------------------
; Copy each section starting from the line above
; Extraction rule title: *** Extract show credits from metadata ***		; non-formal title for user reminder
extraction_type = metadata_nfo												; Type of data to extract (streamindex/metadata_nfo supported)
extraction_line_rule_1 := "WM/MediaCredits :"										; The "fingerprint" data in the ffprobe's report that distinguishes the selected line from every other line. Try to find a unique string.
extraction_line_rule_2 := ""										; Optional: User can fill here another rule that is REQUIRED TO COEXIST WITH rule_1 on the SAME LINE for the same extraction process (logical "AND").
line_exclusion_rule_1 := ""											; Optional: User can fill here one string that causes the line to be skipped if found. Handy, if line_rule_1 and line_rule_2 are inadequate.
stream_include_or_exclude := ""										; Used by ffmpeg to set a "negative mapping" (exclusion) if "exclude"
metadata_xml_header := "credits"											; Required only for .nfo metadata extraction, gives the field an appropriate header in the target .nfo -file. Don't fill <> here, only the text.
extractionrule_filename_iff := ""									; Sets a single string to look for in each processed filename, and apply this rule in only those files. (Logical iff)
extractionrule_filename_ifnot := ""									; Sets a single string to look for in each processed filename, and don't apply this rule with those files. (Logical if not)
extractionrule_ffprobereport_iff := ""								; Sets a single string to look for in ffprobe's report for each processed file, and apply this rule in only those files. (Logical iff)
extractionrule_ffprobereport_ifnot := ""							; Sets a single string to look for in ffprobe's report for each processed file, and don't apply this rule with those files. (Logical if not)
...
extraction_type = metadata_nfo												; Type of data to extract (streamindex/metadata_nfo supported)
extraction_line_rule_1 := "WM/SubTitleDescription:"										; The "fingerprint" data in the ffprobe's report that distinguishes the selected line from every other line. Try to find a unique string.
extraction_line_rule_2 := ""										; Optional: User can fill here another rule that is REQUIRED TO COEXIST WITH rule_1 on the SAME LINE for the same extraction process (logical "AND").
line_exclusion_rule_1 := ""											; Optional: User can fill here one string that causes the line to be skipped if found. Handy, if line_rule_1 and line_rule_2 are inadequate.
stream_include_or_exclude := ""										; Used by ffmpeg to set a "negative mapping" (exclusion) if "exclude"
...
Loop, %dir_rec%\*.%extension_rec%
{
	...
	IfInString, A_LoopFileName, _CONVERTED
	{
		...
		; The line above is commented out, since it only produces non-informational logging.
		continue
	}
...
Loop, %dir_rec%\*.%extension_rec%
{
	...
	IfInString, A_LoopFileName, _CONVERTED
	{
		; The line below was commented out on 240315, as the script was tested to be functional and this only causes a lot of non-informational logging each time the script runs.
		...
		If (days_keepold >= 0)
		{
			...
			{
			; The line below was commented out on 200315, as the script was tested to be functional and this only causes a lot of non-informational logging each time the script runs.
